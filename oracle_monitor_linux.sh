...
if ! which sqlplus >/dev/null 2>&1; then
    ...
    # Common Oracle binary locations  #CHANGE THIS
    ...
    if ! which sqlplus >/dev/null 2>&1; then
        ...
        FOUND_HOMES=$(find /u01 /opt /ora* -name sqlplus 2>/dev/null | grep -v "instant" | grep -v "client" | head -1)
...
if ! which sqlplus >/dev/null 2>&1; then
    ...
    read -p "ORACLE_HOME=" user_oracle_home
...
<body>
    <h1>Oracle Database Monitoring Report V10</h1>
...
add_system_section() {
    ...
    df -h 2>/dev/null | while IFS= read -r line; do
        ...
        if echo "$line" | grep -iE "100%|9[0-9]%" > /dev/null; then
    ...
    if which lsnrctl >/dev/null 2>&1; then
        ...
        echo "<pre>" >> $OUTPUT_FILE
        lsnrctl status 2>/dev/null | while IFS= read -r line; do
            line=$(echo "$line" | sed 's/&/\&amp;/g; s/</\&lt;/g; s/>/\&gt;/g')
...
get_oracle_sids() {
    # Method 1: Check from running processes
    ps_output=$(ps -ef 2>/dev/null | grep -E "[o]ra_pmon|[p]mon_" | awk '{print $NF}' | sort)
    
    if [ -n "$ps_output" ]; then
        echo "$ps_output" | sed -E 's/.*_(.+)$/\1/g'
        return
    fi
    ...
    read -p "Please enter an Oracle SID manually: " manual_sid
    echo "$manual_sid"
}
...
EOF
    ...
    # Execute SQL commands for this SID
    timeout 300s $SQLPLUS -S "/ as sysdba" << EOF >> $OUTPUT_FILE
set pagesize 1000
set linesize 200
set feedback off
set heading on
